...
export default function ViewProcess(props) {
  ...
  // 1. Função para remover a extensão do arquivo (ex: .pdf, .jpeg)
  const formatFileName = (name) => {
    if (!name) return ''
    return name.replace(/\.[^/.]+$/, "")
  }

  // 2. Função para limpar o texto da descrição (Remove "Cite..." e "")
  const cleanDescription = (text) => {
    if (!text) return ''
    return text
      // Remove padrões de colchetes como
      .replace(/\+\]/g, '')
      // Remove frases que começam com "Cite" ou "cite"
      .replace(/\bcite\b[^.!?]*([.!?]|$)/gi, '')
      .trim()
  }

  // 3. Lógica para ordenar: Atestado Técnico primeiro, depois ART, depois o resto
  const sortedDocuments = [...documents].sort((a, b) => {
    const nameA = a.name.toLowerCase()
    const nameB = b.name.toLowerCase()

    if (nameA.includes('atestado') && !nameB.includes('atestado')) return -1
    if (!nameA.includes('atestado') && nameB.includes('atestado')) return 1
    if (nameA.includes('art') && !nameB.includes('art')) return 1
    if (!nameA.includes('art') && nameB.includes('art')) return -1
    return 0
  })
              ...
              <div className="w-full xl:w-1/3 flex flex-col gap-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Busque a ART e faça o upload dos arquivos.</p>
                  </div>
                ...
                <div className="bg-red-50 border-2 border-red-200 p-6 rounded-2xl shadow-inner flex gap-4 items-start">
                  <i className="ph-fill ph-warning-octagon text-4xl text-red-600 mt-1 flex-shrink-0"></i>
                  <div>
                    <h3 className="text-lg font-bold text-red-900">Atenção Importante!</h3>
                    <p className="text-sm text-red-800 mt-1 font-medium">
                      O <span className='font-bold underline'>Atestado Técnico</span> e o <span className='font-bold underline'>Contrato</span> devem ser enviados em arquivos <span className='font-bold'>PDF separados</span>. Não os digitalize no mesmo documento.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full xl:w-2/3 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col min-h-[700px]">
                ...
                {uploadState === 'done' ? (
                  <>
                    ...
                    <div className="flex-1 bg-slate-200 rounded-xl border border-slate-300 overflow-hidden relative min-h-[600px] shadow-inner">
                      {selectedDoc?.url ? (
                        <iframe
                          src={selectedDoc.url}
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-slate-500">
                          ...
                          <p className="font-medium text-lg text-slate-600">Visualizador de Documentos</p>
                          <p className="text-sm">Exibindo: {formatFileName(selectedDoc?.name)}</p>
                        </div>
                      )}
                    </div>
                  </>
                ...
                {uploadState === 'done' && (
                  <button 
                    ...
                    className={`px-6 py-3 text-white rounded-xl font-semibold shadow-sm transition-all flex items-center gap-2 ${
                      reportStatus === 'sent' ? 'bg-blue-600' : 'bg-green-600 hover:bg-green-700'
                    }`}
                  >
                    {reportStatus === 'idle' && <>Gerar e Enviar Relatório</>}
                    {reportStatus === 'sending' && <>Enviando...</>}
                    {reportStatus === 'sent' && (
                      <>
                        <i className="ph ph-check-circle text-xl"></i>
                        Enviado via WhatsApp e E-mail!
                      </>
                    )}
                  </button>
                )}
                    ...
                    <div key={step.id} className="bg-slate-50 border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow">
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          {/* Descrição limpa via função cleanDescription */}
                          <p className="text-sm text-slate-600 mt-1">
                            {cleanDescription(step.desc)}
                          </p>
                          ...
                          <div className="mt-4">
                            ...
                            {step.showComment && (
                              <div className="mt-3">
                                <textarea 
                                  className="w-full text-sm p-4 border rounded-xl"
                                  rows="3"
